...
class SimulationResult:
    ...
    tags: str = ""  # JSON string of alpha tags
    # Status
    success: bool = False
    error_message: str = ""
    alpha_id: str = ""
...
class SimulatorTester:
    ...
    def _extract_operator_hints(self, expression: str, limit: int = 3) -> List[str]:
        """Extract compact operator hints for alpha naming."""
        if not expression:
            return []

        operators = []
        seen = set()
        for token in re.findall(r'\b([A-Za-z_][A-Za-z0-9_]*)\s*\(', expression):
    ...
    def _extract_field_hints(self, expression: str, operators: List[str], limit: int = 2) -> List[str]:
        """Extract likely data field hints while ignoring operators and keywords."""
        if not expression:
            return []

        ignored = {
            'nan', 'true', 'false', 'and', 'or', 'not', 'if', 'else',
            'filter', 'dense', 'rate', 'std', 'constant', 'driver',
            'industry', 'subindustry', 'sector', 'country', 'market'
        }
        ignored.update(operators)

        fields = []
        seen = set()
        for token in re.findall(r'\b[A-Za-z_][A-Za-z0-9_]*(?:\.[A-Za-z_][A-Za-z0-9_]*)?\b', expression):
            normalized = token.lower()
            ...
            if re.fullmatch(r'd\d+', normalized):
                continue
            if normalized not in seen:
                fields.append(token.split('.')[-1][:24])
                seen.add(normalized)
            if len(fields) >= limit:
                break
        return fields

    def _build_alpha_properties(
        self,
        template: str,
        region: str,
        settings: SimulationSettings,
        ...
        """Build deterministic name and description for a completed alpha."""
        tags = tags or []
        operators = self._extract_operator_hints(template)
        fields = self._extract_field_hints(template, operators)
        digest = hashlib.sha1(template.encode('utf-8')).hexdigest()[:8]

        operator_hint = "-".join(operators[:2]) if operators else "expr"
        field_hint = "-".join(fields[:1]) if fields else "multi"
        name = f"Gen2 {region} {operator_hint} {field_hint} {digest}"
        name = re.sub(r'\s+', ' ', name).strip()[:80]

        status = "PASS" if is_success else "WARN"
        checks_summary = self._summarize_checks(checks_data)
        expression = re.sub(r'\s+', ' ', template).strip()

        description_parts = [
            "Generated by Generation Two.",
            f"Status: {status}.",
            f"Region: {region}.",
            ...
            f"Sharpe: {self._format_float(is_data.get('sharpe'))}.",
            f"Fitness: {self._format_float(is_data.get('fitness'))}.",
            f"Turnover: {self._format_float(is_data.get('turnover'))}.",
        ]
        if checks_summary:
            description_parts.append(f"Checks: {checks_summary}.")
        if tags:
    ...
    def _format_float(self, value) -> str:
        """Format API metric values for compact alpha descriptions."""
        try:
            return f"{float(value):.4f}"
        except (TypeError, ValueError):
            return "n/a"

    def _summarize_checks(self, checks_data: List, limit: int = 3) -> str:
        """Create a short human-readable check summary."""
        if not checks_data:
            return ""

        summaries = []
        for check in checks_data[:limit]:
            if isinstance(check, dict):
                ...
                status = check.get('status') or check.get('result') or check.get('severity') or ''
                if status:
                    summaries.append(f"{name}={status}")
                else:
                    summaries.append(str(name))
            else:
                summaries.append(str(check)[:80])

        if len(checks_data) > limit:
            summaries.append(f"+{len(checks_data) - limit} more")
        return ", ".join(summaries)

    def _check_name(self, check) -> str:
        """Return a stable display name for a WorldQuant check."""
        if isinstance(check, dict):
            return str(
                check.get('name')
                or check.get('title')
    ...
    def _check_state(self, check) -> str:
        """Normalize WorldQuant check result/status/severity into one token."""
        if isinstance(check, dict):
            # WorldQuant's current IS checks use "result"; when it exists it
            # should dominate older status/severity fields.
            for key in ('result', 'status', 'severity', 'type'):
                value = check.get(key)
                if value:
                    return str(value).strip().upper()
            return ""

        check_upper = str(check).upper()
        for state in (
            'FAILED', 'FAIL', 'ERROR', 'RED', 'REJECTED', 'INVALID',
            'WARNING', 'WARN', 'PENDING', 'PASS', 'PASSED'
        ):
            if state in check_upper:
                return state
        return ""

    def _classify_checks_by_state(self, checks_data: List) -> Dict[str, List]:
        """Split checks into blocking, warning, pending, and passing groups."""
        classified = {
            'failed': [],
            'warning': [],
            'pending': [],
        ...
        failed_states = {'FAIL', 'FAILED', 'ERROR', 'RED', 'REJECTED', 'INVALID'}
        warning_states = {'WARNING', 'WARN', 'YELLOW'}
        pending_states = {'PENDING', 'QUEUED', 'RUNNING', 'IN_PROGRESS'}
        passing_states = {'PASS', 'PASSED', 'OK', 'GREEN'}

        for check in checks_data or []:
            state = self._check_state(check)
            if state in failed_states:
                classified['failed'].append(check)
            elif state in warning_states:
                classified['warning'].append(check)
            elif state in pending_states:
                classified['pending'].append(check)
            elif state in passing_states:
                classified['passed'].append(check)
            else:
                classified['unknown'].append(check)

        return classified

    def _build_check_error_message(self, classified_checks: Dict[str, List], status: str = "") -> str:
        """Build a concise failure reason from classified checks."""
        failed_names = [self._check_name(check) for check in classified_checks.get('failed', [])]
        warning_names = [self._check_name(check) for check in classified_checks.get('warning', [])]

        parts = []
        if failed_names:
            parts.append(f"Failed checks: {', '.join(failed_names)}")
        if warning_names:
            parts.append(f"Warning checks: {', '.join(warning_names)}")
        if not parts and status:
            parts.append(f"Simulation completed with warning status: {status}")

        return "; ".join(parts)

    def _classify_alpha_tags(
        self,
        is_success: bool,
        checks_data: List,
        is_data: Dict
    ) -> List[str]:
        """Classify completed alphas into stable tags for mining workflows."""
        tags = ['candidate'] if is_success else ['near-miss', 'needs-repair']

        classified_checks = self._classify_checks_by_state(checks_data)
        actionable_checks = classified_checks['failed'] + classified_checks['warning']
        checks_text = json.dumps(actionable_checks, ensure_ascii=False).lower() if actionable_checks else ""
        check_categories = {
            'concentration': ['concentration', 'concentrated', 'weight'],
            'turnover': ['turnover'],
            'correlation': ['correlation', 'prod_corr', 'self_corr', 'correlated'],
            'subuniverse': ['subuniverse', 'sub-universe', 'sub_universe'],
            'ladder': ['ladder'],
            'fitness': ['fitness'],
            'sharpe': ['sharpe'],
        }
        for tag, keywords in check_categories.items():
            if any(keyword in checks_text for keyword in keywords):
                tags.append(tag)

        pending_text = json.dumps(classified_checks['pending'], ensure_ascii=False).lower()
        if 'correlation' in pending_text or 'self_corr' in pending_text:
            tags.append('correlation-pending')
        elif classified_checks['pending']:
            tags.append('check-pending')

        try:
            sharpe = float(is_data.get('sharpe') or 0.0)
        except (TypeError, ValueError):
            sharpe = 0.0
        try:
            fitness = float(is_data.get('fitness') or 0.0)
        except (TypeError, ValueError):
            fitness = 0.0
        try:
            turnover = float(is_data.get('turnover') or 0.0)
        except (TypeError, ValueError):
            turnover = 0.0
    ...
    def _update_alpha_properties(
        ...
        """Best-effort update of alpha name and description."""
        if not alpha_id:
            return False
        if not self.template_generator or not hasattr(self.template_generator, 'make_api_request'):
            logger.debug("Cannot update alpha properties: no authenticated request helper")
            return False

        properties = self._build_alpha_properties(
            template=template,
            region=region,
            settings=settings,
            is_data=is_data,
        ...
        payloads = []
        if isinstance(alpha_data.get('properties'), dict):
            payloads.append({'properties': tagged_properties})
            payloads.append({'properties': base_properties})
            payloads.append(tagged_properties)
            payloads.append(base_properties)
        else:
            payloads.append(tagged_properties)
            payloads.append(base_properties)
            payloads.append({'properties': tagged_properties})
        ...
        for payload in payloads:
            ...
            try:
                ...
                logger.warning(f"Failed to update alpha properties for {alpha_id}: {e}")
                return False

            if response is not None and response.status_code in {200, 202, 204}:
                logger.info(f"Updated alpha properties for {alpha_id}: {properties['name']}")
                return True
            ...
            logger.debug(
                "Alpha properties update attempt failed for %s: status=%s response=%s",
                alpha_id,
                status_code,
                response_text
            )

        logger.warning(f"Could not update alpha properties for {alpha_id}; API rejected known payload formats")
        return False
            ...
            try:
                ...
                if status in complete_statuses or status in warning_statuses:
                    ...
                    if alpha_response.status_code == 200:
                        ...
                        classified_checks = self._classify_checks_by_state(checks_data)
                        has_blocking_checks = (
                            bool(classified_checks['failed'])
                            or bool(classified_checks['warning'])
                            or status in warning_statuses
                        )
                        is_success = not has_blocking_checks
                        check_message = ""
                        if has_blocking_checks:
                            check_message = self._build_check_error_message(
                                classified_checks,
                                status if status in warning_statuses else ""
                            )

                        tags = self._classify_alpha_tags(
                            is_success=is_success,
                            checks_data=checks_data,
                            is_data=is_data
                        )
    ...
    def _has_warnings_only(self, checks_data: List) -> bool:
        ...
        classified_checks = self._classify_checks_by_state(checks_data)
        return bool(classified_checks['failed'] or classified_checks['warning'])
